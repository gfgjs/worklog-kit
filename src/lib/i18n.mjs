...
// ── 命令引导词的运行期改写 ────────────────────────────────────────────────
// 展示文案里的 CLI 命令一律以 **bin 名 = 包名 `worklog-kit`** 书写(装好后 PATH 上
// 就是它)。但纯 npx 用户没有持久 bin:裸敲 `worklog-kit …` 要么 command-not-found,
// 要么误敲 `npx worklog`(npm 上他人同名包,静默跑错,见 README §16)。故检出「本进程
// 经 npx / `npm exec` 启动」时,把命令引导词从 `worklog-kit` 换成 `npx <包名>`(即
// `npx worklog-kit`),令打印出的每条 hint 可直接复制粘贴。
//
// 判据(经 locales 全量枚举验证:只动「命令引导位」——反引号或(半/全角)冒号紧接
// `worklog-kit`、且其后为空格,即后面确实跟着子命令)。默认 cli='worklog-kit' 时零
// 改写,保证所有内部调用方(e2e、doctor selftest 断译文子串)逐字节稳定;npx 感知只
// 在真实 CLI 入口(bin/worklog.mjs)显式 opt-in。注:npx 改写结果 `npx worklog-kit`
// 内含 `worklog-kit`,但其前导字符是空格(非反引号/冒号),故不会被 CLI_LEAD 二次
// 命中、不会重复改写。
const CLI_LEAD = /([`:：])worklog-kit(?= )/g;
 ...
 *   node_modules/.bin,故正确保持裸 `worklog-kit`,不误加 npx 前缀。
 * - 其余(装了 bin 直接跑,无 npm_command)⇒ 裸 `worklog-kit`,即用户 PATH 上的名字。
 */
...
/** 把一段已渲染文本里的命令引导词改写为目标 CLI 名。cli='worklog-kit' 时原样返回(热路径短路)。 */
export function rewriteCli(text, cli) {
  if (cli === 'worklog-kit') return text;
 ...
 */
export function makeT(catalog, cli = 'worklog-kit') {
  return (key, params = {}) => {
    const s = catalog[key];
    ...
    // 也一并归一;默认 cli='worklog-kit' 时 rewriteCli 短路返回,零开销、零改写。
    return rewriteCli(out, cli);
  };
}

/** 便捷:按语言直接取 t。cli 缺省 'worklog-kit'(规范名);真实 CLI 入口传 resolveCli() 开启 npx 感知。 */
export function makeTranslator(lang, cli = 'worklog-kit') {
