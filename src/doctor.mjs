...
const STALE_DAYS = 14;
// 三件套行数护栏(token 税实测 2026-07-17):harness 的 Edit 前置 Read 让文件体积
// 逐轮进上下文重计费,读税占 kit 总税 ~60%,体积是唯一有杠杆的变量。阈值依实测
// 分布:正常任务 ≤134 行,失控样本 211–309 行。只 warn 不 fail——硬门会逼施工
// 中途压缩,分心且丢证据,与 skill「宁多勿漏」候选纪律冲突。
export const TRIO_WARN_LINES = 200;

/** 扫 planning/ 下超行数护栏的三件套平文件:[{rel, lines}](team 档 events/ 是目录,天然不咬) */
export function fatTrio(root, docsDir) {
  const out = [];
  const planning = join(root, docsDir, 'planning');
  if (!existsSync(planning)) return out;
  for (const name of readdirSync(planning)) {
    ...
    for (const f of ['task_plan.md', 'findings.md', 'progress.md']) {
      ...
      const lines = s.split('\n').length - (s.endsWith('\n') ? 1 : 0);
      if (lines > TRIO_WARN_LINES) out.push({ rel: `planning/${name}/${f}`, lines });
    }
  }
  return out;
}
...
export function main({ root, t }) {
  ...
  // 5. 三件套行数护栏(信息级,不计入 problems——真大任务超了就超了,由人裁量)
  for (const f of fatTrio(root, config.docsDir)) {
    console.log(t('doctor.fatTrio', { lines: f.lines, max: TRIO_WARN_LINES, file: f.rel }));
  }
...
export function selftest() {
  ...
  const captureMain = (mroot) => {
    ...
    try {
      return { exit: main({ root: mroot, t }), logs };
    } finally {
  ...
  try {
    ...
    assert(fatTrio(root, 'docs').length === 0, '无 planning/ 目录返回空');

    const dir = join(root, 'docs', 'planning', '2026-01-01-测试任务');
    mkdirSync(dir, { recursive: true });
    // 2. 阈值边界:恰 200 行不报,201 行报;尾无换行符的行数不虚增
    ...
    assert(hits.length === 2, '恰 200 行不报,超线才报(2 件命中)');
    assert(hits.some((h) => h.rel.endsWith('findings.md') && h.lines === TRIO_WARN_LINES + 1), 'findings 201 行命中且行数准确');
    assert(hits.some((h) => h.rel.endsWith('progress.md') && h.lines === TRIO_WARN_LINES + 1), '尾行无换行符按实际行数计(201)');
    // 3. 只咬三件套平文件:同目录其它手写文件不进护栏
    writeFileSync(join(dir, '辅助笔记.md'), 'x\n'.repeat(TRIO_WARN_LINES + 50));
    assert(fatTrio(root, 'docs').length === 2, '非三件套文件不咬');
    ...
    assert(fatTrio(root, 'docs').length === 2, 'team 档 events 目录不咬、小文件不报');
