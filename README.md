...
完整工作轮集中落盘，多轮实测用结果表，接续只读当前所需材料；交接或中断时及时保存。写作约定与工具边界见[记录纪律](docs/usage.md#记录纪律)。
...
样例的 `state.md` 停在「施工 / T2」，`details.md` 给出 T2 的设计、范围、验证与回交条件，并用结果表记录已有自检证据。把 Skill 与样例交给另一个模型，即可演练一次不用工具的接续。
